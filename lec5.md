...
这是一个D触发器例子，D读取，Q输出
输入D的数据要在时钟到达之前就稳定，在到达之后也要保持一段时间稳定，在Clock-to-Q（锁存延迟）之后Q的值和D保持一致
保持时间一般大于锁存延迟
因此是有延迟存在的

状态元件通常有**寄存器**和**存储器**两种
- 寄存器：有一个写使能信号（WE），当他是0的时候时钟边沿到来但是输出不变，如果是1那么输出开始变为输入
...
— 读操作：地址address有效之后，经过**取数时间**，data out上的数据变为有效
...
### 数据通路的设计
注意，所有图中的线路**都是有数据**的，而不是说没有数据，所以我们通过**多路选择器**来选择符合我们当前指令的功能的值，并且参与执行
...
在存储器里面的Adr是通过ALU的加运算得到的，可以看lw指令的具体含义
...
多周期处理器的好处：
- 时钟周期短（绝对时间，以前是一整条指令，现在是最长的一个阶段）
- 不同指令所用周期数**可以不同**
...
这个是取指令阶段结束的时候，也就是下一个时钟周期到来，新的值PC+4写入到PC当中，而不会更新指令，所以IR指令寄存器不变，他的写使能信号为0
...
这些之后，就要根据操作码功能码来译码，判断指令类型。还有一个重要的**投机运算**
![1761202025580](image/lec5/1761202025580.png)
投机运算：将PC的值和立即数的值符号扩展之后相加，新的值送到BranchTarget寄存器
因为还不能判断这是一个Branch指令，所以先放在这里，等判断是不是Branch指令再决定是否更新PC，这样可以省去一个周期（如果是Branch指令的话）
本来计算BranchTarget的时候是PC+4+imm16<<2
但是因为PC已经更新了，所以直接用PC的值加imm16<<2就行
...
ALU的输出要保持稳定，Rw，busW都要稳定的，所以RegDst保持是1，然后RegWr=1，写使能信号为1了，这个时候进行写入
也就是说为了解决竞争问题我们需要第四个周期，来完成这个写入操作
...
这个周期进行地址计算，计算出存储器的地址，得到的地址可能是读取地址也可能是写入地址

#### sw指令的存数周期：第四个周期
![1761204080193](image/lec5/1761204080193.png)
把rt寄存器的内容写入存储器，所以MemWr=1，其他写使能信号都为0
...
RegWr=1，进行写入操作，其他控制信号保持不变，维持稳定
所以lw指令比sw还要多一个周期！
...
根据$\mu$PC取出微指令存入$\mu$IR，顺序执行的时候每次$\mu$PC+1

取指令和译码用专门的微程序实现，称为**取指微程序**
![1761206661650](image/lec5/1761206661650.png)
微命令和微操作是一一对应的，比如说多路选择器的一个信号是1（微命令），就相当于选择多路选择器的一条路这个操作

##### 微指令格式的设计
![1761206842584](image/lec5/1761206842584.png)
![1761206877120](image/lec5/1761206877120.png)
微命令就是控制信号，地址相当于状态编号，微地址是11相当于下一个要取11号状态

##### 微操作码编码方式
###### 不译法
每个bit都写在这个op字段，一位对应一个微命令
![1761207041858](image/lec5/1761207041858.png)
不译法的微操作码部分就是由控制信号组成，比如这里有17个控制信号，那就有17个bit的微操作码
这样用不译法，不必译码，并行能力强
但是这样的话微指令字可能会非常长，而且编码的空间利用率低

可以用字段直接编码法
把微指令分成多个字段，每个字段有对应的微命令编码
如果微命令可以同时执行，那么就是相容的，如果不能同时执行就是互斥的
把互斥的微命令放在同一字段，相容的放在不同字段

## 带异常处理的处理器设计
![1761207427620](image/lec5/1761207427620.png)
![1761207605425](image/lec5/1761207605425.png)
数据通路：
![1761207620912](image/lec5/1761207620912.png)
加入两个寄存器
-EPC：保存断点时的PC值，也就是异常发生时的下一条指令地址
-Cause：保存异常类型

需加入两个寄存器的“写使能”控制信号
EPCWr：保存断点时该信号有效，使**断点PC写入EPC**
CauseWr：在处理器发现异常（如：非法指令、溢出）时该信号有效，使异常类型被写到Cause寄存器
• 需一个控制信号IntCause来选择正确的值写入到Cause中
• 需将异常查询程序入口地址（MIPS为0x8000 0180）写入PC，可在原PCSource控制的多路器中再增加一路，其输入为0x8000 0180。

在有限状态机中**增加异常处理的状态**，每种异常占一个状态
![1761207813356](image/lec5/1761207813356.png)
![1766760058066](image/lec5/1766760058066.png)