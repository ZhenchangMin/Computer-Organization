...
$$
s是符号位，M是尾数，B是基数，E是阶码
尾数的最高有效位（二进制）必须是1，因此可以省略不存储，这称为**隐含的1**
左规：尾数左移，阶码递减，比如0.00101 × 2^5 = 1.01 × 2^2
右规：尾数右移，阶码递增，比如101.0 × 2^3 = 1.01 × 2^5

通过左规和右规将非规格化的浮点数规格化。
规格化：得到尽量**多的有效数位**，使浮点数表示具有**唯一性**

注意，如果不是IEEE754，规格化都是变成0.1xxxxxx的形式，而不是1.xxxxxx的形式。
